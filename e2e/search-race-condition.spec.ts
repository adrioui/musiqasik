...
test.describe("Search race condition fix", () => {
  test("typing quickly should only show final search results", async ({
    page,
  }) => {
    ...
    const resultNames = await resultsList
      .locator("p.truncate")
      .allTextContents();

    // Verify "Radiohead" is in results
    const hasRadiohead = resultNames.some((name) =>
      name.toLowerCase().includes("radiohead"),
    );
  ...
  test("search works normally with regular typing", async ({ page }) => {
    ...
    const resultNames = await resultsList
      .locator("p.truncate")
      .allTextContents();
    const hasColdplay = resultNames.some((name) =>
      name.toLowerCase().includes("coldplay"),
    );
