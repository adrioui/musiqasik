...
export class LastFmAuthError extends Data.TaggedError(
  "musiqasik/LastFmAuthError",
)<{
...
export class LastFmAuthService extends Context.Tag(
  "musiqasik/LastFmAuthService",
)<
  LastFmAuthService,
  {
    readonly getSession: (
    ...
    Effect.gen(function* () {
      ...
      const data = yield* Effect.tryPromise({
        try: () =>
          ...
          new LastFmAuthError({ message: "Failed to parse response" }),
      });
...
export const LastFmAuthServiceLive = Layer.effect(
  LastFmAuthService,
  makeLastFmAuthService,
);
