...
## 2024-05-24 - Interaction: Media Sliders
**Learning:** Binding a slider directly to media playback time causes jitter during dragging as the player updates the value while the user tries to set it.
**Action:** Use a local state for the slider value while dragging (via `onValueChange`) and only commit the seek action to the player on release (`onValueCommit`). This ensures smooth interaction.
