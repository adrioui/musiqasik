...
export const svgToPng = (svgElement: SVGSVGElement) =>
  Effect.async<Blob, ExportError>((resume) => {
    ...
    if (!ctx) {
      resume(Effect.fail(new ExportError({ message: 'Failed to get canvas context' })))
      return
    }
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        if (blob) {
          ...
          resume(Effect.fail(new ExportError({ message: 'Failed to create blob' })))
        }
      }, 'image/png')
    }
...
export const copyToClipboard = (text: string) =>
  Effect.tryPromise({
    ...
    catch: (error) => new ExportError({ message: 'Failed to copy to clipboard', cause: error }),
  })

// Legacy Promise-based API for backwards compatibility
export async function svgToPngLegacy(svgElement: SVGSVGElement): Promise<Blob | null> {
