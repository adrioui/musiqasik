...
export class LastFmService extends Context.Tag("LastFmService")<
  LastFmService,
  {
    ...
    getArtistInfo: (
      artistName: string,
    ) => Effect.Effect<Artist | null, AppError>;
    getSimilarArtists: (
      artistName: string,
...
export class DatabaseService extends Context.Tag("DatabaseService")<
  DatabaseService,
  {
    ...
    ) => Effect.Effect<Artist, AppError>;
    getCachedEdges: (
      artistId: string,
    ) => Effect.Effect<
      Array<{ target: Artist; match_score: number }>,
      AppError
    >;
    upsertEdges: (
      edges: Array<{
...
export class GraphService extends Context.Tag("GraphService")<
  GraphService,
  {
    buildGraph: (
      ...
      GraphData & { metrics?: { duration: number; nodeCount: number } },
      AppError
    >;
