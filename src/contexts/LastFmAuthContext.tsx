...
const exchangeToken = (token: string) =>
  Effect.gen(function* () {
    ...
    if (!response.ok) {
      return yield* Effect.fail(new NetworkError({ message: `Server error: ${response.status}` }))
    }

    const data = yield* Effect.tryPromise({
      try: () => response.json() as Promise<{ sessionKey: string; username: string }>,
      catch: (error) => new NetworkError({ message: 'Invalid server response', cause: error }),
    })
