...
describe("useGenreColors", () => {
  ...
  it("should handle mixed valid and invalid tags", () => {
    ...
    expect(result.current.getNodeColor(nodes[0])).not.toBe(
      "hsl(var(--graph-node))",
    );
    expect(result.current.getNodeColor(nodes[1])).toBe(
      "hsl(var(--graph-node))",
    );
    expect(result.current.getNodeColor(nodes[2])).toBe(
      "hsl(var(--graph-node))",
    );
  });
