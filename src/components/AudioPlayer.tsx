...
const AlbumArt = memo(function AlbumArt({ track }: { track: Track }) {
  return (
...
const TrackTitle = memo(function TrackTitle({ track }: { track: Track }) {
  return (
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-lg">
      <div className="glass-panel-pill p-2 pl-3 pr-6 flex items-center gap-4">
        ...
        <Controls isPlaying={isPlaying} onTogglePlay={togglePlay} onFavorite={onFavorite} />
      </div>
    </div>
  )
}
